...
/**
 * Full End-to-End Integration Test for Investment & Portfolio Platform
 */
@SpringBootTest
@ActiveProfiles("test")
public class InvestmentEndToEndIntegrationTest {

    @Autowired(required = false)
    private InvestmentService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Investment & Portfolio Platform")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            InvestmentDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            InvestmentDTO fetched = service.getByReferenceCode(refCode);
