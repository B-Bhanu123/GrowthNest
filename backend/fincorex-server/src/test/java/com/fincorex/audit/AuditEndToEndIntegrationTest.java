...
/**
 * Full End-to-End Integration Test for Immutable Audit Logging
 */
@SpringBootTest
@ActiveProfiles("test")
public class AuditEndToEndIntegrationTest {

    @Autowired(required = false)
    private AuditService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Immutable Audit Logging")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            AuditDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            AuditDTO fetched = service.getByReferenceCode(refCode);
