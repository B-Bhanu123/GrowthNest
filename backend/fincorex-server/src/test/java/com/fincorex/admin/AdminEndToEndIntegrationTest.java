...
/**
 * Full End-to-End Integration Test for Admin & Operations Center
 */
@SpringBootTest
@ActiveProfiles("test")
public class AdminEndToEndIntegrationTest {

    @Autowired(required = false)
    private AdminService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Admin & Operations Center")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            AdminDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            AdminDTO fetched = service.getByReferenceCode(refCode);
