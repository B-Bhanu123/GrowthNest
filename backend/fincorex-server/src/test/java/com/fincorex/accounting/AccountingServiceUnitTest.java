...
public class AccountingServiceUnitTest {
    ...
    private AccountingRecordEntity entity;
    private UUID ownerId;

    @BeforeEach
    void setUp() {
        ownerId = UUID.randomUUID();
        entity = new AccountingRecordEntity("REF-10029", ownerId, new BigDecimal("450.00"), "ACTIVE");
        entity.setId(UUID.randomUUID());
    }

    @Test
    void testCreateRecordSuccess() {
        when(repository.save(any(AccountingRecordEntity.class))).thenReturn(entity);

        AccountingDTO result = service.createRecord("REF-10029", ownerId, new BigDecimal("450.00"), "ACTIVE");
    ...
    void testGetByReferenceCodeSuccess() {
        ...
        AccountingDTO result = service.getByReferenceCode("REF-10029");
