...
/**
 * Full End-to-End Integration Test for Identity & Access Management
 */
@SpringBootTest
@ActiveProfiles("test")
public class IdentityEndToEndIntegrationTest {

    @Autowired(required = false)
    private IdentityService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Identity & Access Management")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            IdentityDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            IdentityDTO fetched = service.getByReferenceCode(refCode);
