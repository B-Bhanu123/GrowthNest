...
public class FraudServiceUnitTest {
    ...
    private FraudRecordEntity entity;
    private UUID ownerId;

    @BeforeEach
    void setUp() {
        ownerId = UUID.randomUUID();
        entity = new FraudRecordEntity("REF-10029", ownerId, new BigDecimal("450.00"), "ACTIVE");
        entity.setId(UUID.randomUUID());
    }

    @Test
    void testCreateRecordSuccess() {
        when(repository.save(any(FraudRecordEntity.class))).thenReturn(entity);

        FraudDTO result = service.createRecord("REF-10029", ownerId, new BigDecimal("450.00"), "ACTIVE");
    ...
    void testGetByReferenceCodeSuccess() {
        ...
        FraudDTO result = service.getByReferenceCode("REF-10029");
