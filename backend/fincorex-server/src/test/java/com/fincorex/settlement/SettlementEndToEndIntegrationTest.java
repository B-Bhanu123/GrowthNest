...
/**
 * Full End-to-End Integration Test for Merchant Batch Settlement
 */
@SpringBootTest
@ActiveProfiles("test")
public class SettlementEndToEndIntegrationTest {

    @Autowired(required = false)
    private SettlementService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Merchant Batch Settlement")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            SettlementDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            SettlementDTO fetched = service.getByReferenceCode(refCode);
