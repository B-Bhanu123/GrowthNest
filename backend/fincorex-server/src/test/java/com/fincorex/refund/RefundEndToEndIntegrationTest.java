...
/**
 * Full End-to-End Integration Test for Refund Management
 */
@SpringBootTest
@ActiveProfiles("test")
public class RefundEndToEndIntegrationTest {

    @Autowired(required = false)
    private RefundService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Refund Management")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            RefundDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            RefundDTO fetched = service.getByReferenceCode(refCode);
