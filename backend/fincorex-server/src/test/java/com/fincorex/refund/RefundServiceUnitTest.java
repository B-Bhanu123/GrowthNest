...
public class RefundServiceUnitTest {
    ...
    private RefundRecordEntity entity;
    private UUID ownerId;

    @BeforeEach
    void setUp() {
        ownerId = UUID.randomUUID();
        entity = new RefundRecordEntity("REF-10029", ownerId, new BigDecimal("450.00"), "ACTIVE");
        entity.setId(UUID.randomUUID());
    }

    @Test
    void testCreateRecordSuccess() {
        when(repository.save(any(RefundRecordEntity.class))).thenReturn(entity);

        RefundDTO result = service.createRecord("REF-10029", ownerId, new BigDecimal("450.00"), "ACTIVE");
    ...
    void testGetByReferenceCodeSuccess() {
        ...
        RefundDTO result = service.getByReferenceCode("REF-10029");
