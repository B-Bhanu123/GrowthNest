...
/**
 * Full End-to-End Integration Test for Transaction Processing Core
 */
@SpringBootTest
@ActiveProfiles("test")
public class TransactionEndToEndIntegrationTest {

    @Autowired(required = false)
    private TransactionService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Transaction Processing Core")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            TransactionDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            TransactionDTO fetched = service.getByReferenceCode(refCode);
