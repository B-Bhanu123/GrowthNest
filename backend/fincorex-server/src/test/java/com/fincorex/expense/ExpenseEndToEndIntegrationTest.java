...
/**
 * Full End-to-End Integration Test for Corporate Expense Management
 */
@SpringBootTest
@ActiveProfiles("test")
public class ExpenseEndToEndIntegrationTest {

    @Autowired(required = false)
    private ExpenseService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Corporate Expense Management")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            ExpenseDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            ExpenseDTO fetched = service.getByReferenceCode(refCode);
