...
/**
 * Full End-to-End Integration Test for Stored-Value Digital Wallet
 */
@SpringBootTest
@ActiveProfiles("test")
public class WalletEndToEndIntegrationTest {

    @Autowired(required = false)
    private WalletService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Stored-Value Digital Wallet")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            WalletDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            WalletDTO fetched = service.getByReferenceCode(refCode);
