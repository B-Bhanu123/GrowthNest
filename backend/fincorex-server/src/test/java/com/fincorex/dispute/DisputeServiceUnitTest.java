...
public class DisputeServiceUnitTest {
    ...
    private DisputeRecordEntity entity;
    private UUID ownerId;

    @BeforeEach
    void setUp() {
        ownerId = UUID.randomUUID();
        entity = new DisputeRecordEntity("REF-10029", ownerId, new BigDecimal("450.00"), "ACTIVE");
        entity.setId(UUID.randomUUID());
    }

    @Test
    void testCreateRecordSuccess() {
        when(repository.save(any(DisputeRecordEntity.class))).thenReturn(entity);

        DisputeDTO result = service.createRecord("REF-10029", ownerId, new BigDecimal("450.00"), "ACTIVE");
    ...
    void testGetByReferenceCodeSuccess() {
        ...
        DisputeDTO result = service.getByReferenceCode("REF-10029");
