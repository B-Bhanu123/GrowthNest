...
/**
 * Full End-to-End Integration Test for Merchant Acquiring Management
 */
@SpringBootTest
@ActiveProfiles("test")
public class MerchantEndToEndIntegrationTest {

    @Autowired(required = false)
    private MerchantService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Merchant Acquiring Management")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            MerchantDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            MerchantDTO fetched = service.getByReferenceCode(refCode);
