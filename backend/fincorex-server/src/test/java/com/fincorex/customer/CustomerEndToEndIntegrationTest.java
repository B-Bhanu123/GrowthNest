...
/**
 * Full End-to-End Integration Test for Customer & Account Management
 */
@SpringBootTest
@ActiveProfiles("test")
public class CustomerEndToEndIntegrationTest {

    @Autowired(required = false)
    private CustomerService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Customer & Account Management")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            CustomerDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            CustomerDTO fetched = service.getByReferenceCode(refCode);
