...
/**
 * Full End-to-End Integration Test for Insurance Policy System
 */
@SpringBootTest
@ActiveProfiles("test")
public class InsuranceEndToEndIntegrationTest {

    @Autowired(required = false)
    private InsuranceService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Insurance Policy System")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            InsuranceDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            InsuranceDTO fetched = service.getByReferenceCode(refCode);
