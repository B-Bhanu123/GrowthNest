...
/**
 * Full End-to-End Integration Test for Lending & Underwriting Engine
 */
@SpringBootTest
@ActiveProfiles("test")
public class LendingEndToEndIntegrationTest {

    @Autowired(required = false)
    private LendingService service;

    @Test
    @DisplayName("Validate full E2E lifecycle flow for Lending & Underwriting Engine")
    void testFullLifecycleFlow() {
        ...
        if (service != null) {
            LendingDTO created = service.createRecord(refCode, ownerId, amount, "ACTIVE");
            assertNotNull(created);
            assertEquals(refCode, created.getReferenceCode());
            assertEquals("ACTIVE", created.getStatus());

            LendingDTO fetched = service.getByReferenceCode(refCode);
