package com.fincorex.customer.cache;
...
/**
 * Distributed Cache Adapter for Customer & Account Management (Customer)
 */
@Component
public class CustomerCacheManager {
    ...
    private final Map<String, CustomerDTO> cacheStore = new ConcurrentHashMap<>();

    public void put(String referenceCode, CustomerDTO dto) {
        log.debug("[CACHE-PUT] Caching DTO for ref: {}", referenceCode);
        cacheStore.put(referenceCode, dto);
    }

    public Optional<CustomerDTO> get(String referenceCode) {
        CustomerDTO hit = cacheStore.get(referenceCode);
        if (hit != null) {
    ...
    public void clearAll() {
        log.info("[CACHE-CLEAR] Clearing all cached entries for customer");
        cacheStore.clear();
    }
}
