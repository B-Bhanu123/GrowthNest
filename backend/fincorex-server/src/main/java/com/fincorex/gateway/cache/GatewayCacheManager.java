package com.fincorex.gateway.cache;
...
/**
 * Distributed Cache Adapter for API Gateway & Security Proxy (Gateway)
 */
@Component
public class GatewayCacheManager {
    ...
    private final Map<String, GatewayDTO> cacheStore = new ConcurrentHashMap<>();

    public void put(String referenceCode, GatewayDTO dto) {
        log.debug("[CACHE-PUT] Caching DTO for ref: {}", referenceCode);
        cacheStore.put(referenceCode, dto);
    }

    public Optional<GatewayDTO> get(String referenceCode) {
        GatewayDTO hit = cacheStore.get(referenceCode);
        if (hit != null) {
    ...
    public void clearAll() {
        log.info("[CACHE-CLEAR] Clearing all cached entries for gateway");
        cacheStore.clear();
    }
}
